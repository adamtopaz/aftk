...
export interface CreateAFTKToolsOptions {
	cwd?: string;
	projectRoot?: string;
	requestTimeoutMs?: number;
}

export interface AFTKToolset {
...
export function createAFTKTools(options: CreateAFTKToolsOptions = {}): AFTKToolset {
	const cwd = options.cwd ?? process.cwd();
	const projectRoot = options.projectRoot ?? findProjectRoot(cwd);
	const client = new AftkHubClient(projectRoot, options.requestTimeoutMs ?? DEFAULT_REQUEST_TIMEOUT_MS);
	...
	const tools: ToolDefinition[] = [
		{
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					...
					const result = await request<OpenResult>("open", { path: filePath }, signal);
					const text = result.opened
						? `Opened file worker: ${result.path}`
						: `File already open: ${result.path}`;
					return {
		...
		},
		{
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					const filePath = normalizePath(params.path);
					const result = await request<CloseResult>("close", { path: filePath }, signal);
		...
		},
		{
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					const filePath = normalizePath(params.path);
					const result = await request<LoadNodeResult>(
						"load_node",
						{
							path: filePath,
							line: params.line,
		...
		},
		{
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					const filePath = normalizePath(params.path);
					const result = await request<HoverResult | null>(
						"get_hover",
						{
							path: filePath,
							line: params.line,
		...
		},
		{
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					const filePath = normalizePath(params.path);
					const result = await request<PlainGoalResult | null>(
						"get_plain_goal",
						{
							path: filePath,
							line: params.line,
		...
		},
		{
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					const filePath = normalizePath(params.path);
					const result = await request<PlainTermGoalResult | null>(
						"get_plain_term_goal",
						{
							path: filePath,
							line: params.line,
		...
		},
		{
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					const filePath = normalizePath(params.path);
					const result = await request<InfoViewResult>(
						"get_infoview",
						{
							path: filePath,
							line: params.line,
		...
		},
		{
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					const filePath = normalizePath(params.path);
					const result = await request<GetGoalsResult>("get_goals", { path: filePath, id: params.id }, signal);
					return {
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					...
					const result = await request<RunTacticResult>(
						"run_tactic",
						{ path: filePath, id: params.id, tactic: params.tactic },
						signal,
					);
			...
			async execute(_toolCallId, params: any, signal) {
				try {
					...
					const result = await request<RunTacticStepsResult>(
						"run_tactic_steps",
						{ path: filePath, id: params.id, tactics: params.tactics },
						signal,
					);
